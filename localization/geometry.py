...
def drawC(cA, ax):
    ...
    for q, w in enumerate(cA):
        ...
        if t2 > ymax:
            ymax = t2
        circ = pl.Circle((w.c.x, w.c.y),  radius=w.r,
                         fc='none', lw=2, ec=pickColor(q))
        ax.add_artist(circ)
...
class vec:
    ...
    def __str__(self):
        return 'vec(%s, %s, %s)' % (self.dx, self.dy, self.dz)
...
class Triangle:
    ...
    def __str__(self):
        return 'Triangle{%s, %s, %s}' % (self.a.__str__().strip('p'),
                                         self.b.__str__().strip('p'),
                                         self.c.__str__().strip('p'))
